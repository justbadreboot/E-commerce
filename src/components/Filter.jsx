...
const Filter =()=>{
  ...
  return(
    <div className="relative mx-auto py-4 sm:py-16 px-4 w-full max-w-7xl bg-gray-50 font-poppins">
      <div className="grid grid-cols-4 gap-y-8 gap-x-4">
        <div className="col-span-full pb-6 flex flex-col sm:flex-row items-center justify-between space-y-5 sm:space-y-0 border-b border-gray-200">
          <h2 className="text-3xl text-ternary-60 font-bold">Búsqueda de Productos</h2>
          <div className="flex items-center space-x-5">
            <div className="hidden lg:inline-block relative">
              <input type="search" id="search" name="search"
                placeholder="Buscar"
                className="form-input pl-11 pr-5 w-44 block shadow-sm rounded-full border-gray-300 bg-gray-50 text-sm placeholder-gray-300 focus:border-primary-100 focus:ring-1 focus:ring-blue-300"
              />
            ...
            <ul className="flex flex-col items-start space-y-2">
              {subCategories.map(subcategory => (
                <li key={subcategory.name}>
                  <a href={subcategory.href} className="text-base text-gray-700 font-semibold hover:text-blue-400">{subcategory.name}</a>
                </li>
              ))
              }
            </ul>
        ...
        <div className="col-span-full lg:col-span-3">
          ...
          <div className="border-2 border-gray-200 rounded-lg h-96 lg:h-full" />
        </div>
      </div>
    </div>
  )
