...
const RelatedProductCard =({product})=>{
    return(
        <div key={product.id} className="col-span-1 relative py-4 md:py-5 px-5 shadow-sm rounded border border-gray-200 bg-white overflow-hidden hover:shadow-lg font-poppins">
            ...
            <button onClick={()=> addToCart('dani',product.id,1, product.pvp, product.name)} className={`bg-green-500 py-2 px-4 rounded-lg border-2 border-gray-200 text-xs text-white font-semibold hover:text-white hover:border-transparent`}>
                Añadir al carrito
            </button>
        </div>
    )
